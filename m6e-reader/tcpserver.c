...
// >>>>>>>>>>>>>>>>>>>>>>>>>>			
			...
			if(Board == MYD)
				...
				count = UART_write2port(serial_fd, buffer, count);
			if(count < 0)
			{
...
// <<<<<<<<<<<<<<<<<<<<<<<<<<<<
			if(isReading == 0)
			{
				...
				if(Board == MYD)
					...
					count = UART_read_from_port(serial_fd, buffer, MAX_BUFFER_SIZE);
...
//		plog("tcp_sendBytes count=%d", count);
	...
	if(stop == 2)
	{
		...
		if(Board == MYD)
			...
			count = UART_write2port(serial_fd, buffer, 8);
